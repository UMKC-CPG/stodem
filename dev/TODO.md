...
| 39 | Engagement redesign v2 (sigmoid government channels, per-citizen randomized constants, neutral naming) | `gaussian.py`, `citizen.py`, `stodem.py`, `stodem.in.toml`, `jobs/*`, `DESIGN.md`, `PSEUDOCODE.md`, `BEHAVIOR_GUIDE.md` | Both government channels are now logistic sigmoids: aversion-match of `-I(Pca,Pge)` (up), preference-gap of the `preference_alignment = I(Pcp,Pge)` shortfall (down). Each midpoint = per-citizen `frac · A_max`, where `A_max = matched_self_overlap()` (band ceiling, recomputed each step). ALL engagement constants are now per-citizen `(center, spread)` draws: `negativity_bias` (was `threat_weight`; Gaussian center 2.0, floored 1.0), `govt_engagement_scale` (was `govt_engagement_rate`), `aversion_match`/`preference_gap` `*_midpoint_frac` + `*_steepness` (replace `sat_ref`). Renamed anger→aversion-match, resignation→preference-gap, satisfaction→preference_alignment; dropped emotional "threat" wording. Centers/spreads are tuning seeds; calibrate against smallTest using `dev/BEHAVIOR_GUIDE.md` (2026-06-18) |
